var Grid=function(obj,w,h,t){
    ...
    this.importColors=function(array,x,y){
        function pos2(x2,y2){
    ...
    this.importImages=function(array,x,y){
        function pos2(x2,y2){
               return y2*array.width-array.width-1+x2;
        }
        function isInBounds2(a,b){
        ...
        for(let i=1;i<=array.width;i++){
            for(let j=1;j<=array.height;j++){
                if(isInBounds2(i+x,j+y)){
                    this.setImage(i,j,array.data[pos2(i+x,j+y)].substring(0,7));
                }
            }
        }
    };
